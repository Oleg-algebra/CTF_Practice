...
for line in file:
    for char in line:
        if char in string.printable:
            chipher_text+=char
file.close()
file1=open('encrypt_message.txt','w')
file1.write(chipher_text)
file1.close()
print(string.printable[:-2])
print(chipher_text)


def xor(str1, str2):
    if len(str1) != len(str2):
        return "XOR EXCEPTION: Strings are not of equal length!"
...
def single_byte_xor(plaintext, key):
    if len(key) != 1:
        return "KEY LENGTH EXCEPTION: In single_byte_xor key must be 1 byte long!"
    return xor(plaintext, key*len(plaintext))

def hamming_distance(str1, str2):
    result = xor(str1, str2)
    #print(result)
    return bin( int( codecs.encode(result,'hex'), 16) ).count('1')
...
def find_xor_keysize( ciphertext, hamming_blocks, minsize=2, maxsize=10 ):
    ...
    if (hamming_blocks*maxsize) > len(ciphertext):
        return "OUT OF BOUND EXCEPTION! Lower the hamming_blocks or the key maxsize!"
...
def break_repeat_key_xor( ciphertext ):
    ...
    for ks in key_sizes:
        ...
        for key in itertools.product(*all_keys,repeat=1):
            print(key)
...
break_repeat_key_xor(chipher_text)